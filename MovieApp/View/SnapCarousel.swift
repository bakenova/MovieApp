...
struct SnapCarousel<Content: View, T: Identifiable>: View {
    ...
    func getOffset(item: T, width: CGFloat) -> CGFloat {
        ...
        let previous = getIndex(item: item) == currentIndex - 1 ? (offset < 0 ? topOffset : -topOffset) : 0
        
        let next = getIndex(item: item) == currentIndex + 1 ? (offset < 0 ? topOffset : -topOffset) : 0
        
        let checkBetween = currentIndex >= 0 && currentIndex < list.count ? (getIndex(item: item) - 1 == currentIndex - 1 ? previous : next) : 0
