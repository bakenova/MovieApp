...
struct Films: View {
        ...
        NavigationView{
            VStack{
                ...
                ZStack{
                    ...
                    Profile(isUserCurrentlyLoggedOut: $isUserCurrentlyLoggedOut).opacity(self.currentTab == "Profile" ? 1 : 0)
                }
            }
        }
    }
    ...
    func customBlock(collectionName: String, movies: [Movie]) -> some View {
        ZStack {
            Rectangle()
                .fill(Color.white.opacity(self.scheme == .dark ? 0.1 : 1))
                .cornerRadius(15)
                .padding(.bottom, 10)
            ...
            VStack {
                HStack {
                    ...
                    NavigationLink(destination: FilmListView(viewModel: viewModel, films: movies)) {
                        Text("See More")
    ...
    func BGView() -> some View{
        GeometryReader{ proxy in
            ...
            TabView(selection: $currentIndex) {
                ForEach(viewModel.recommendations.indices, id: \.self) { index in
                    WebImage(url: URL(string: viewModel.recommendations[index].imageName))
                        .resizable()
